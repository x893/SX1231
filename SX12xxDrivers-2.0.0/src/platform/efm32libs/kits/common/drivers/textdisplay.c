                         ...
                         TEXTDISPLAY_Handle_t  *handle)
{
	...
	if (charBufferTbl[deviceNo].lines > textdisplay->displayDevice.geometry.height / FONT_HEIGHT)
...
EMSTATUS TEXTDISPLAY_WriteChar(TEXTDISPLAY_Handle_t handle, char c)
{
	...
	if (textdisplay->scrollEnable)
	{
		...
		*/
		if ((textdisplay->xpos == 0) && (textdisplay->ypos == textdisplay->lines-1))
		{
...
EMSTATUS TEXTDISPLAY_WriteString(TEXTDISPLAY_Handle_t handle, char * str)
{
	...
	while ((c = *str++) != 0)
	{
		...
		/* Clear screen if we start at top left position and scroll is not enabled. */
		if ((textdisplay->xpos == 0)
		&& (textdisplay->ypos == 0)
		&& (!textdisplay->scrollEnable))
		{
			TextdisplayClear(textdisplay);
			textdisplay->updateMode  = TEXTDISPLAY_UPDATE_MODE_FULL;
		}
		...
		/* Update display if we reach the end of the line or a full display update is required. */
 ...
 *
 *****************************************************************************/
static bool TextdisplayEscapeSequence(TEXTDISPLAY_Device_t* textdisplay, int c)
{
#define ASCII_ESC                           (27)
...
#define TEXTDISPLAY_ESC_SEQNO_CURSOR_LEFT_ONE_CHAR	(5)
	...
	if (ASCII_ESC == c)
	{
		/* Clear escape sequence, store the ESC character, update mode none and return true. */
	...
	else
	{
		...
		*pChar = c;
		...
		for (seqNo = 0; seqNo < ESC_SEQ_MAX; seqNo++)
		{
			...
			if (strncmp(escapeSequence, escapeSequences[seqNo], len) == 0)
			{
				/* We match part of the cursor home sequence. Set anyMatch and update mode to none. */
				anyMatch = true;
				textdisplay->updateMode = TEXTDISPLAY_UPDATE_MODE_NONE;
 ...
 *
 *****************************************************************************/
static void TextdisplayCharAdd(TEXTDISPLAY_Device_t* textdisplay, int c)
{
	switch (c)
	{
	case '\r':  /* check for CR */
	...
	case '\b':  /* check for bell character, changes color to red */
		if ( (DISPLAY_COLOUR_MODE_MONOCHROME != textdisplay->displayDevice.colourMode)
		&&   (DISPLAY_COLOUR_MODE_MONOCHROME_INVERSE != textdisplay->displayDevice.colourMode) )
		{
	...
	textdisplay->charArray[textdisplay->ypos][textdisplay->xpos] = c - FONT_ASCII_START;
 ...
 * @return  EMSTATUS code of the operation.
 *****************************************************************************/
static EMSTATUS TextdisplayLineDraw(TEXTDISPLAY_Device_t*  textdisplay, unsigned int y)
{
	unsigned int x, i;
	uint8_t      c;
	...
	for (i = 0; i < FONT_HEIGHT; i++)
	{
		...
		if (textdisplay->displayDevice.colourMode == DISPLAY_COLOUR_MODE_MONOCHROME_INVERSE)
		{
			int usedBits;
			int startPixel = x * FONT_WIDTH;
			...
			memset (&rowPtr[startPixel>>FONT_BITS_LOG2], 0xff, (textdisplay->displayDevice.geometry.width - startPixel) / 8);
		}
#endif

		rowPtr += textdisplay->displayDevice.geometry.stride / (sizeof(FontBits_t) * 8);
	}
  
	return textdisplay->displayDevice.pPixelMatrixDraw(
											&textdisplay->displayDevice,
											textdisplay->lineBuffer,
											0,
...
static EMSTATUS TextdisplayUpdate(TEXTDISPLAY_Device_t*  textdisplay)
{
	...
	case TEXTDISPLAY_UPDATE_MODE_FULL:
		...
		case DISPLAY_ADDRESSING_BY_ROWS_ONLY:
			...
			if (textdisplay->lines * FONT_HEIGHT < textdisplay->displayDevice.geometry.height)
			{
				switch (textdisplay->displayDevice.colourMode)
				{
				case DISPLAY_COLOUR_MODE_MONOCHROME:
					memset (textdisplay->lineBuffer, 0x00, textdisplay->displayDevice.geometry.width / 8);
					break;
				case DISPLAY_COLOUR_MODE_MONOCHROME_INVERSE:
					memset (textdisplay->lineBuffer, 0xff, textdisplay->displayDevice.geometry.width / 8);
					break;
				default:
				...
				for (i = textdisplay->lines * FONT_HEIGHT;
					...
					i++)
				{
					status = textdisplay->displayDevice.pPixelMatrixDraw(
															&textdisplay->displayDevice,
															textdisplay->lineBuffer,
															0,
															textdisplay->displayDevice.geometry.width,
